<style scoped lang="less">
  @import "./coffee-order.less";
</style>

<template>
  <div class="coffee_order">
    <!-- 顶部导航 -->
    <yd-navbar class="coffee_order_title" title="订单详情">
      <div @click="$router.back()" slot="left">
        <yd-navbar-back-icon>返回</yd-navbar-back-icon>
      </div>
    </yd-navbar>
    <yd-flexbox direction="vertical" class="coffee_order_content">
      <yd-flexbox-item class="coffee_order_content_harder">
        <div class="payment">
          <span class="payment_left" slot="left">{{order.state_display}}</span>
          <span class="payment_right" slot="right">订单编号：{{order.order_sn}}</span>
        </div>
      </yd-flexbox-item>
      <yd-flexbox-item class="coffee_order_content_about">
        <div class="title">
        ...
        <div class="price">
          ...
          <div class="price_unit">
            <div class="left">咖啡单价</div>
            <div class="right"><em>¥</em>{{product.product_price}}x{{product.product_quantity}}</div>
          </div>
        </div>
      </yd-flexbox-item>
      <yd-flexbox-item class="coffee_order_content_info">
        <div class="card">
          <div class="card_main">
            <div class="card_main_item" v-for="(info, key) in information" :key="key">
          ...
          <div class="card_foot">
            <div v-if="order.state !== 'unpaid'" class="card_foot_cert">
              ...
              <div class="card_foot_cert_action">
                ...
                <div v-if="order.state === 'paid'" class="cert_button" @click="coffeeRefund">申请退款</div>
              </div>
            </div>
            <div class="card_foot_item" v-for="(item, key) in explain" :key="key">
            ...
            <div class="card_foot_item">
              ...
              <div class="right">{{product.product_name}} {{product.product_quantity}} 杯</div>
            </div>
          </div>
        </div>
      </yd-flexbox-item>
      <!--<yd-flexbox-item class="coffee_order_content_user">-->
        <!--<div class="item">-->
          <!--<div class="left">-->
    ...
    <a v-if="order.state === 'unpaid'" class="coffee_order_button" :href="'/order/pay/'+order.order_sn">
      确认支付
    </a>
  </div>
</template>
