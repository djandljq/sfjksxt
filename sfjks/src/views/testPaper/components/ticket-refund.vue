<style scoped lang="less">
  @import "./ticket-refund.less";
</style>

<template>
  <div class="ticket_refund">
    <!-- 顶部导航 -->
    <yd-navbar class="ticket_refund_title" title="退款详情">
      <div @click="$router.back()" slot="left">
        <yd-navbar-back-icon>返回</yd-navbar-back-icon>
      </div>
    </yd-navbar>
    <yd-flexbox class="ticket_refund_content" direction="vertical">
      <yd-flexbox-item class="ticket_refund_content_title">
        <div class="ticket_title">{{title}}</div>
        <div class="ticket_desc">{{about}}</div>
      </yd-flexbox-item>
      <yd-flexbox-item class="ticket_refund_content_quantity">
        <div class="left">退款数量</div>
        <div class="right">
          <yd-spinner :max="max" unit="1" v-model="quantity"></yd-spinner>
        </div>
      </yd-flexbox-item>
      <yd-flexbox-item class="ticket_refund_content_price">
        <div class="left">可退金额</div>
        <div class="right"><em>¥</em>{{refund_price}}</div>
      </yd-flexbox-item>
      <yd-flexbox-item class="ticket_refund_content_method">
    ...
    <div class="ticket_refund_desc">
      <div class="desc_title">退款原因</div>
      <textarea class="desc_content" v-model="desc" placeholder="请输入退款原因" maxlength="200"></textarea>
    </div>
    <yd-button @click.native="handleRefundClick" class="ticket_refund_button" size="large" bgcolor="#36CB9A" color="#FFF">申请提交</yd-button>
  </div>
</template>
